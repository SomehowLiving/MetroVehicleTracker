  ...
  const handleVehicleNumberChange = (value: string) => {
    ...
    if (vehicleMode === "checkout" && upperValue.length > 0) {
      const selectedVehicle = activeVehicles?.find(
        (v: any) => v.vehicleNumber.toUpperCase() === upperValue,
      );

      if (selectedVehicle) {
        // Vehicle found - populate all details
        ...
        setSelectedCheckinId(selectedVehicle.id);
        setIsVehicleFound(true);
      } else {
        // Vehicle not found - clear all details
        ...
        setSelectedCheckinId(undefined);
        setIsVehicleFound(false);
        setClosingKm(undefined); // Clear closing KM too
      }
    }
  };
