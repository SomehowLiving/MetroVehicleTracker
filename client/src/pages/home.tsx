...
export default function Home() {
  ...
  const totalVehicles =
    stores?.reduce((sum, store) => sum + store.vehiclesIn, 0) || 0;
  const totalStores = stores?.length || 0;
  const todaysCheckins =
    stores?.reduce(
      (sum, store) => sum + store.vehiclesIn + store.vehiclesOut,
      0,
    ) || 0;
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Vehicle Attendance & Tracking System
                </p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-blue-800 max-w-2xl mx-auto">
            Digital gate log management system for all 32 Metro stores. Track
            vehicle movements, manage driver data, and maintain real-time
            attendance records.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white/10 backdrop-blur-sm border-white/20">
            <CardContent className="p-6 text-center">
              <div className="text-3xl font-bold text-grey mb-2">
                {totalStores}
              </div>
          ...
          <Card className="bg-white/10 backdrop-blur-sm border-white/20">
            <CardContent className="p-6 text-center">
              <div className="text-3xl font-bold text-metro-yellow mb-2">
                {totalVehicles}
              </div>
          ...
          <Card className="bg-white/10 backdrop-blur-sm border-white/20">
            <CardContent className="p-6 text-center">
              <div className="text-3xl font-bold text-grey-800 mb-2">98.5%</div>
